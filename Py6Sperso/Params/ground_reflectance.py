...
%f %f %f\n""" % (par1, par2, par3)
    ...
    @classmethod
    def HomogeneousOcean(cls, wind_speed, wind_azimuth, salinity, pigment_concentration, rfoam, xfoam):
        ...
        - rfoam := foam intrinsic reflectance
        - xfoam := foam relative surface
        """
        return """0 Homogeneous surface
1 (directional effects)
6 (Ocean BRDF)
%f %f %f %f %f %f\n""" % (wind_speed, wind_azimuth, salinity, pigment_concentration, rfoam, xfoam)
...
%d %d %d\n""" % (leaf_dist, hot_spot, lai, hot_spot_param, leaf_reflec, leaf_trans, soil_albedo)
    ...
    @classmethod
    def HomogeneousVerstaeteEtAl(cls, kappa_param, phase_funct, scattering_type, leaf_area_density, sun_flecks_radius,
                                 ssa, legendre_first, legendre_second, k1, k2, asym_factor, chil):
...
2 (Verstraete Pinty Dickinson model\n"""
    ...
    @classmethod
    def HomogeneousKuuskMultispectralCR(cls, lai, lad_eps, lad_thm, relative_leaf_size, chlorophyll_content,
                                        leaf_water_equiv_thickness, effective_num_layers, ratio_refractive_indices,
...
9 (Kuusk's multispectral CR model)\n"""
        ...
        bottom = "%f %f %f %f %f\n" % (
        chlorophyll_content, leaf_water_equiv_thickness, effective_num_layers, ratio_refractive_indices,
        weight_first_price_function)
